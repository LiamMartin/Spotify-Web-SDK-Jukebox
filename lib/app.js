...
config.auth = require('.' + config.spotify.authDir + 'auth.json');
...
spotifyApi.setAccessToken(config.auth.access);
spotifyApi.setRefreshToken(config.auth.refresh_token);

tokenManager = new TokenManager(spotifyApi, io);
...
io.on('connection', function (socket) {
    ...
    socket.emit('connectAccessToken', spotifyApi.getAccessToken());
    ...
    socket.on("playNext", () => {
        ...
        changeSong(queue[queue_index].uri);
        queue_index++;
    });
...
var changeSong = function (uri) {
    spotifyApi.play({uris: [uri]}).then((data) => {
    });
}